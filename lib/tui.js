...
async function selectMany(title, items, opts = {}) {
  ...
  if (!isInteractive()) {
    ...
    const dflt = pre.size ? [...pre].map((i) => i + 1).join(",") : "all";
    const a = (await ask(`Select (comma list, 'all', or Enter for ${dflt}): `)).trim();
    if (!a) return (pre.size ? [...pre] : items.map((_, i) => i)).map((i) => items[i].value);
    if (a.toLowerCase() === "all") return items.map((it) => it.value);
