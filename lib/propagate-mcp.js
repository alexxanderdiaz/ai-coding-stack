...
// Inspect a tool's MCP config so a wizard can decide merge vs fresh+backup.
function configInfo(tool) {
  const t = TARGETS[tool];
  if (!t) return { known: false };
  const exists = fs.existsSync(t.file);
  const cfg = readJson(t.file);
  const servers = (cfg[t.key] && typeof cfg[t.key] === "object") ? cfg[t.key] : {};
  const otherServers = Object.keys(servers).filter((k) => !SERVERS[k]);
  return { known: true, exists, file: t.file, otherServers };
}

// opts.fresh: back up the existing config and reset the MCP key (keeps other top-level keys),
// so only the toolkit's servers remain. Default = merge (preserve existing servers).
function propagate(tool, opts = {}) {
  const t = TARGETS[tool];
  if (!t) { log(`  ! unknown tool for MCP: ${tool}`); return; }
  const cfg = readJson(t.file);
  if (opts.fresh && fs.existsSync(t.file) && !DRY) {
    const bak = `${t.file}.bak-${tsStamp()}`;
    fs.copyFileSync(t.file, bak);
    log(`  ↩ backed up ${path.basename(t.file)} -> ${path.basename(bak)}`);
    cfg[t.key] = {};
  }
  ...
  if (DRY) { log(`  would write ${t.file} (${t.key}: ${added.join(", ")})${opts.fresh ? " [fresh+backup]" : ""}`); return; }
...
module.exports = { SERVERS, TARGETS, propagate, configInfo };
