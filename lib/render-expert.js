...
const TOOLS = {
  ...
  // Global: skills ~/.zcode/skills/<id>/SKILL.md (VERIFIED — same frontmatter as Claude/opencode).
  // Agents ~/.zcode/cli/agents/<id>.md — frontmatter MD; format inferred from the Claude/opencode
  // subagent convention (no .md sample shipped) — review the first one under Settings → Subagents.
  zcode: {
    scope: "global", dirName: ".zcode",
    skillSub: id => `skills/${id}/SKILL.md`, agentSub: id => `cli/agents/${id}.md`,
    renderAgent: (meta, body) => {
      const fm = ["---", `name: ${meta.id}`, `description: ${yamlScalar(meta.description)}`];
      if (meta.tools) fm.push(`tools: ${yamlScalar(meta.tools)}`);
      fm.push("---");
